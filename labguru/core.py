...
class Labguru(object):
    ...
    #define another constructor to use token
    def __init__(self, token, url):
        self.session = Session(token=token, url=url)
    ...
    def add_project(self, title, description=None):
        ...
        #expand all the attributes of self.session
        return Project(token=self.session.token, url=self.session.url, title=title, description=description).register()

    def get_project(self, project_id):
        validate_required_fields(action='get project', project_id=project_id)
        proj = Project(id=project_id, token=self.session.token, url=self.session.url)
        return proj.get()

    def find_projects(self, name):
        return Project(token=self.session.token, url=self.session.url).list(name=name)
    ...
    def update_project(self, project_id, title, description=None, **kwargs):
        ...
        proj = Project(token=self.session.token, url=self.session.url, id=project_id, title=title, description=description, **kwargs)
        return proj.update()
    ...
    def list_projects(self, page_num):
        return Project(token=self.session.token, url=self.session.url).list(page_num=page_num)
    ...
    def add_folder(self, project_id, title, description=None):
        ...
        return Folder(token=self.session.token, url=self.session.url, project_id=project_id, title=title, description=description).register()

    def get_folder(self, folder_id):
        validate_required_fields(action='get folder', folder_id=folder_id)
        return Folder(token=self.session.token, url=self.session.url, id=folder_id).get()

    def find_folders(self, name):
        return Folder(token=self.session.token, url=self.session.url).list(name=name)
    ...
    def update_folder(self, folder_id, title, description=None, **kwargs):
        ...
        return Folder(token=self.session.token, url=self.session.url, id=folder_id, title=title, description=description, **kwargs).update()
    ...
    def list_folders(self, project_id=None, page_num=None):
        if project_id is not None:
            ...
            return [Folder(token=self.session.token, url=self.session.url, project_id=project_id, **milestone) for milestone in milestones]
        elif page_num is not None:
            return Folder(token=self.session.token, url=self.session.url).list(page_num=page_num)
        else:
    ...
    def add_experiment(self, project_id, folder_id, title, description=None):
        ...
        return Experiment(token=self.session.token, url=self.session.url, project_id=project_id, milestone_id=folder_id,
    ...
    def get_experiment(self, experiment_id):
        ...
        return Experiment(token=self.session.token, url=self.session.url, id=experiment_id).get()

    def find_experiments(self, name):
        return Experiment(token=self.session.token, url=self.session.url).list(name=name)
    ...
    def update_experiment(self, experiment_id, title, description=None, **kwargs):
        ...
        return Experiment(token=self.session.token, url=self.session.url, id=experiment_id, title=title, description=description,
                          **kwargs).update()
    ...
    def list_experiments(self, folder_id=None, page_num=None):
        if folder_id is not None:
            ...
            return [Experiment(token=self.session.token, url=self.session.url, milestone_id=folder_id, **experiment)
                    for experiment in experiments]
        elif page_num is not None:
            return Experiment(token=self.session.token, url=self.session.url).list(page_num=page_num)
        else:
    ...
    def add_experiment_procedure(self, container_id, name, section_type='text', container_type='Projects::Experiment',
        ...
        return Procedure(token=self.session.token, url=self.session.url,
    ...
    def find_experiment_procedures(self, name):
        return Procedure(token=self.session.token, url=self.session.url).list(name=name)

    def get_experiment_procedure(self, section_id):
        validate_required_fields(action='get section', section_id=section_id)
        return Procedure(token=self.session.token, url=self.session.url, id=section_id).get()
    ...
    def list_experiment_procedures(self, experiment_id=None, page_num=None):
        if experiment_id is not None:
            ...
            return [Procedure(token=self.session.token, url=self.session.url, container_id=experiment_id, **experiment['experiment_procedure'])
                    for experiment in experiment_procedures]
        elif page_num is not None:
            return Procedure(token=self.session.token, url=self.session.url).list(page_num=page_num)
        else:
    ...
    def add_element(self, section_id, data=None, element_type='text', container_type='ExperimentProcedure', **kwargs):
        ...
        return Element(token=self.session.token, url=self.session.url,
    ...
    def find_elements(self, name):
        return Element(token=self.session.token, url=self.session.url).list(name=name)

    def get_element(self, element_id):
        validate_required_fields(action='get element', element_id=element_id)
        return Element(token=self.session.token, url=self.session.url, id=element_id).get()

    def update_element(self, element_id, name, **kwargs):
        validate_required_fields(action='update element', element_id=element_id)
        return Element(token=self.session.token, url=self.session.url, id=element_id, name=name, **kwargs).update_element()
    ...
    def list_elements(self, section_id=None, page_num=None):
        if section_id is not None:
            ...
            return [Element(token=self.session.token, url=self.session.url, container_id=section_id, **element)
                    for element in elements]
        elif page_num is not None:
            return Element(token=self.session.token, url=self.session.url).list(page_num=page_num)
        else:
            raise ValueError('Either experiment_id or page_num must be specified')
    ...
    def get_elements_by_type(self, experiment_id, element_type):
        ...
        return Element(token=self.session.token, url=self.session.url, experiment_id=experiment_id, element_type=element_type).list_by_type()
    ...
    def add_inventory_item(self, name, item_type):
        ...
        return InventoryItem(token=self.session.token, url=self.session.url, name=name, item_type=item_type).register()
    ...
    def get_inventory_item(self, item_id, item_type):
        ...
        item = InventoryItem(id=item_id, token=self.session.token, url=self.session.url, item_type=item_type)
        return item.get()

    def find_inventory_items(self, name, item_type):
        validate_names(action='find inventory items', item_type=item_type)
        return InventoryItem(token=self.session.token, url=self.session.url, item_type=item_type).list(name=name)
    ...
    def update_inventory_item(self, item_id, name, item_type, **kwargs):
        ...
        return InventoryItem(token=self.session.token, url=self.session.url, id=item_id, name=name, item_type=item_type, **kwargs).update()

    def list_inventory_items(self, item_type, page_num):
        validate_names(action='list inventory items', item_type=item_type)
        return InventoryItem(token=self.session.token, url=self.session.url, item_type=item_type).list(page_num=page_num)
    ...
    def add_stock(self, stock_name, storage_id, storage_type, stockable_type, stockable_id, **kwargs):
        ...
        return Stock(token=self.session.token, url=self.session.url,
                       name=stock_name,
    ...
    def find_stocks(self, name):
        return Stock(token=self.session.token, url=self.session.url).list(name=name)

    def get_stock(self, stock_id):
        validate_required_fields(action='get stock', stock_id=stock_id)
        return Stock(token=self.session.token, url=self.session.url, id=stock_id).get()

    def update_stock(self, stock_id, **kwargs):
        validate_required_fields(action='update stock', stock_id=stock_id)
        return Stock(token=self.session.token, url=self.session.url, id=stock_id, **kwargs).update()

    def list_stocks(self, page_num):
        return Stock(token=self.session.token, url=self.session.url).list(page_num=page_num)
    ...
    def list_datasets(self, page_num):
        return Datasets(token=self.session.token, url=self.session.url).list(page_num=page_num)
    
    def get_dataset(self, dataset_id):
        validate_required_fields(action='get dataset', dataset_id=dataset_id)
        return Datasets(token=self.session.token, url=self.session.url, id=dataset_id).get()
    ...
    def add_dataset(self, dataset_name, dataset_id, data, **kwargs):
        ...
        return Datasets(token=self.session.token, url=self.session.url,
                       name=dataset_name,
                       id=dataset_id,
